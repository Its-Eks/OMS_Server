...
async function addTrialTemplates() {
  ...
  try {
    ...
    // Trial Email Templates with correct naming convention
    ...
    for (const template of trialTemplates) {
      ...
      console.log(`✅ Added/Updated template: ${template.key}`);
    }

    console.log('🎉 All trial email templates have been added!');
    
  } catch (error) {
    console.error('❌ Error adding trial templates:', error);
  } finally {
    await client.close();
  }
}
